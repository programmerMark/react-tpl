...
export function formatTime(
  timestamp: number | string = 1585734546345,
 ...
 */
export const formatDateToText = (time: string | number, isTimeStamp = false) => {
  let timeString = '';
  if (time) {
    let intervalTime = 0;
    if (!isTimeStamp) {
      intervalTime = Math.floor((Date.now() - Date.parse(time as string)) / 1000);
    } else {
